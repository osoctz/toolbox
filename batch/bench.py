...
async def fetch(session, url):
    ...
    async with session.post(url, json=json_payload, timeout=600) as response:
...
if __name__ == '__main__':
    ...
    # 计算所有请求的最大时间
    max_time_of_request = max(response_times)
    # 计算每秒生成的 token 数量
    ...
    print(f'  max time of request       : {max_time_of_request:.2f} seconds')
