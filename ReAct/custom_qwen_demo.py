...
def count(keyword):
    result = []
    try:
        sql = 'select count(1) as num from t_bu_knowledge_document_ext where content like "%{}%"'.format(keyword)
        # print(sql)
        cursor.execute(sql)
        # 获取所有记录列表
        ...
        # logger.error("rt error: {0}".format(e))
        db.rollback()
    return 0


def similarity_search_with_score_by_vector(vs_path, query):
    """
    相似度查询, 取top_k
    :return:
    """
    # vs_path = ''
    # print(f'{vs_path=}')
    model_path = '/path/models/BAAI_bge-large-zh-v1.5'
...
def call_plugin(plugin_name: str, plugin_args: str) -> str:
    ...
    if plugin_name == 'knowledge_search':
        ...
        vs_path = '/path/gw_title/vector_store/bge-large-zh-v1.5'
        return similarity_search_with_score_by_vector(vs_path, json5.loads(plugin_args)['query'])
    elif plugin_name == 'record_count':

        return count(json5.loads(plugin_args)["query"])
    else:
        raise NotImplementedError
...
def test():
    tools = [
        {
            'name_for_human': '知识库搜索',
            'name_for_model': 'knowledge_search',
            'description_for_model': '知识库搜索是一个知识库检索工具,根据输入的关键字从向量知识库中检索知识。',
            'parameters': [
                {
                    'name': 'query',
        ...
        {
            'name_for_human': '数据统计',
            'name_for_model': 'record_count',
            'description_for_model': '数据统计是一个统计工具, 根据输入关键字从数据库中统计记录总数',
            'parameters': [
                {
                    'name': 'query',
                    'description': '搜索关键词或短语',
    ...
    history = []
    for query in ['你好', '搜索一下棚户区', '再搜下2023年棚户区改造面积', '统计相关棚户区的数据']:
